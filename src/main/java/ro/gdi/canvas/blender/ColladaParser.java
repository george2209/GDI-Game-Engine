...
import ro.gdi.canvas.blender.collada.MaterialParserHelper;
import ro.gdi.canvas.blender.collada.MeshParserHelper;
import ro.gdi.geometry.XYZColor;
import ro.gdi.geometry.XYZVertex;
import ro.gdi.util.BackgroundTask;
    ...
    private void parseMeshObjects(BufferedInputStream inputStream) throws IOException {
        switch (super.iSTATUS_ENGINE) {
            ...
            case PARSER_ENGINE_MESH_VERTEX_PROPERTIES:{
                //uv
                this.iVerticesProperties |= super.getByteArrayAsBoolean(inputStream) ? OpenGLProgramFactory.SHADER_VERTICES_WITH_UV_DATA_MATERIAL : OpenGLProgramFactory.SHADER_UNDEFINED;
                //normal
                this.iVerticesProperties |= super.getByteArrayAsBoolean(inputStream) ? OpenGLProgramFactory.SHADER_VERTICES_WITH_NORMALS : OpenGLProgramFactory.SHADER_UNDEFINED;
                //color per vertex
                this.iVerticesProperties |= super.getByteArrayAsBoolean(inputStream) ? OpenGLProgramFactory.SHADER_VERTICES_WITH_OWN_COLOR : OpenGLProgramFactory.SHADER_UNDEFINED;
                //vertex stride
                iVertexStride = Float.BYTES * (3 + //x,y,z
                        ((this.iVerticesProperties & OpenGLProgramFactory.SHADER_VERTICES_WITH_UV_DATA_MATERIAL) > 0 ? 2 : 0) + //uv
                        ((this.iVerticesProperties & OpenGLProgramFactory.SHADER_VERTICES_WITH_NORMALS) > 0 ? 3 : 0) + //normals
                        ((this.iVerticesProperties & OpenGLProgramFactory.SHADER_VERTICES_WITH_OWN_COLOR) > 0 ? 4 : 0) ); //color
            ...
            case PARSER_ENGINE_VERTEX_INDEX:{
                ...
                this.iVerticesProperties = OpenGLProgramFactory.SHADER_UNDEFINED;
