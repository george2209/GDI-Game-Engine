...
import ro.gdi.geometry.XYZTextureUV;
...
public abstract class AbstractObjParser {
    ...
    protected XYZVertex getByteArrayAsVertex(
        ...
        if((verticesProperties & OpenGLProgramFactory.SHADER_VERTICES_WITH_UV_DATA_MATERIAL) > 0){
            textureUV = new XYZTextureUV(bb.getFloat(index),
                    bb.getFloat(index + Float.BYTES));
            index+= Float.BYTES * 2;
        }
        if((verticesProperties & OpenGLProgramFactory.SHADER_VERTICES_WITH_NORMALS) > 0){
        ...
        if((verticesProperties & OpenGLProgramFactory.SHADER_VERTICES_WITH_OWN_COLOR) > 0){
