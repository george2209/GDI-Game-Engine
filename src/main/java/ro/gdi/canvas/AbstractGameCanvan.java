...
import ro.gdi.canvas.features.CavanMovements;
import ro.gdi.geometry.XYZMaterial;
import ro.gdi.geometry.XYZVertex;
import ro.gdi.lighting.TheSun;
import ro.gdi.util.DebugUtils;
import ro.gdi.util.OpenGLBufferArray3;
import ro.gdi.util.OpenGLProgram;
...
public abstract class AbstractGameCanvan extends CavanMovements {
    ...
    //Light part
    //TODO: don`t like it as Singleton! To be updated as public constructor.
    private TheSun iTheSun = TheSun.getStaticInstance();

    //Textures part

    private int iShaderType = OpenGLProgramFactory.SHADER_UNDEFINED;
...
//        }
    ...
    protected void buildDrawOrderBuffer(final int[] drawOrder){
        if(BuildConfig.DEBUG &&
                this.iShaderType == OpenGLProgramFactory.SHADER_UNDEFINED)
    ...
    private void calculateShaderType(XYZVertex vertex) {
        ...
        this.iShaderType = OpenGLProgramFactory.SHADER_ONLY_VERTICES;

        //RGBA color
        if (vertex.vertexColor != null) {
            this.iShaderType |= OpenGLProgramFactory.SHADER_VERTICES_WITH_OWN_COLOR;
            // R, G, B, A --> 3, 4, 5, 6
        ...
        if(vertex.getUvTexture().isPresent()){
            // U, V --> 7,8
            this.iShaderType |= OpenGLProgramFactory.SHADER_VERTICES_WITH_UV_DATA_MATERIAL;
            //U,V
        ...
        if(vertex.getNormal().isPresent()) {
            this.iShaderType |= OpenGLProgramFactory.SHADER_VERTICES_WITH_NORMALS;
        ...
        if(this.iMaterial != null)
        {
            ...
            if(this.iMaterial.iAmbientFileNameID != OpenGLUtils.INVALID_UNSIGNED_VALUE){
                this.iShaderType |= OpenGLProgramFactory.SHADER_VERTICES_WITH_AMBIENT_TEXTURE;
            }
            //Diffuse texture
            ...
            if(BuildConfig.DEBUG){
                ...
                if(!isCheckOK && (this.iShaderType & OpenGLProgramFactory.SHADER_VERTICES_WITH_UV_DATA_MATERIAL) != 0){
...
//        }
        ...
        if((this.iShaderType & OpenGLProgramFactory.SHADER_VERTICES_WITH_UV_DATA_MATERIAL) != 0) {
            if((this.iShaderType & OpenGLProgramFactory.SHADER_VERTICES_WITH_AMBIENT_TEXTURE) != 0) // || (add here with OR all textures!)
            {
    ...
    public void destroy(){
        ...
        iShaderType = OpenGLProgramFactory.SHADER_UNDEFINED;
        iIndexOrderLength = 0;
        iShaderStride = 0;
    }
