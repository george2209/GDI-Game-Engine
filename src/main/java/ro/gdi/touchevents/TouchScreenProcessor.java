...
//    }
     ...
     * The vector is normalized so the magnitude is ignored and it will only contain..of course the direction.
...
//                "\nobjW=" + result[3]);
            if(this.iEventsListener!=null){
                ...
                throw new AssertionError("No listener to the TouchScreenProcessor!\r\nTo register a listener call addTouchScreenListener().");
            }
        } else {
            if(this.iEventsListener!=null){
                this.iEventsListener.fireTouchClick(getVectorFromDisplayCoordinate(x, y, viewMatrix, projectionMatrix));
            }
        }
    }
