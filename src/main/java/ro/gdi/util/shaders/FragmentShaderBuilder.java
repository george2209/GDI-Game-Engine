...
public class FragmentShaderBuilder extends AbstractShaderBuilder{
    ...
    public FragmentShaderBuilder(){
        //super(shaderType);
        iStrShaderDeclarations.append("\n#version ").append(OpenGLProgramFactory.SHADER_VERSION).append(" es\n");
        iStrShaderDeclarations.append(
    ...
    public FragmentShaderBuilder build(final int shaderType){
        final boolean isColorPerVertex = (shaderType & OpenGLProgramFactory.SHADER_VERTICES_WITH_OWN_COLOR) > 0;
        final boolean isColorGlobal = (shaderType & OpenGLProgramFactory.SHADER_VERTICES_WITH_GLOBAL_DIFFUSE_COLOR) > 0;
        final boolean isNormalPerVertex = (shaderType & OpenGLProgramFactory.SHADER_VERTICES_WITH_NORMALS) > 0;

        if(isNormalPerVertex){
            iStrShaderDeclarations.append("in vec3 ").append(OpenGLProgramFactory.SHADER_VARIABLE_FR_aVectorNormal).append(";\n");
        }

        //prepare this declaration for lightning support
        if((shaderType & OpenGLProgramFactory.SHADER_VERTICES_WITH_UV_DATA_MATERIAL) != 0) {
            iStrShaderDeclarations.append("in vec2 ")
                    .append(OpenGLProgramFactory.SHADER_VARIABLE_FR_aUVTexture).append(";\n");
        }
        //DIFFUSE COLOR IS A MUST
        if(isColorPerVertex){
            iStrShaderDeclarations.append("in vec4 ").append(OpenGLProgramFactory.SHADER_VARIABLE_diffuseMaterialColor).append(";\n");
        } else if(isColorGlobal){
            iStrShaderDeclarations.append("uniform vec4 ").append(OpenGLProgramFactory.SHADER_VARIABLE_diffuseMaterialColor).append(";\n");
        } else if(BuildConfig.DEBUG && ((shaderType & OpenGLProgramFactory.SHADER_VERTICES_WITH_DIFFUSE_TEXTURE) == 0)) {
        ...
        if(isNormalPerVertex) {
            ...
            iStrShaderBody.append("\t\tfragColor = ").append(OpenGLProgramFactory.SHADER_VARIABLE_diffuseMaterialColor).append(";\n");
        }
    ...
    private FragmentShaderBuilder addAmbientLight(final int shaderType){
        ...
        if((shaderType & OpenGLProgramFactory.SHADER_VERTICES_WITH_AMBIENT_TEXTURE) != 0){
            //texture is present so use it.
            iStrShaderDeclarations.append("uniform sampler2D ")
                    .append(OpenGLProgramFactory.SHADER_VARIABLE_ambientTexture)
                    .append(";\n");
            iStrShaderBody.append("\t\t ambientColor *= texture( ")
                    .append(OpenGLProgramFactory.SHADER_VARIABLE_ambientTexture)
                    .append(", ")
                    .append(OpenGLProgramFactory.SHADER_VARIABLE_FR_aUVTexture)
                    .append(").rgba;").append("\n");
        } else if((shaderType & OpenGLProgramFactory.SHADER_VERTICES_WITH_OWN_COLOR) != 0) {
            // use the diffuse parameter "per vertex" that is already declared as diffuse color
            iStrShaderBody.append("\t\t ambientColor *= ")
                    .append(OpenGLProgramFactory.SHADER_VARIABLE_diffuseMaterialColor)
                    .append(";\n");

        } else if((shaderType & OpenGLProgramFactory.SHADER_VERTICES_WITH_GLOBAL_DIFFUSE_COLOR) != 0) {
    ...
    private FragmentShaderBuilder addDiffuseLight(final int shaderType) {
        ...
        iStrShaderBody.append("\t\t vec3 norm = normalize( ").
                append(OpenGLProgramFactory.SHADER_VARIABLE_FR_aVectorNormal).
                append(") ;\n");
        ...
        iStrShaderBody.append("\t\t vec3 lightDir = normalize( -").
                append(OpenGLProgramFactory.SHADER_VARIABLE_diffuseLightDirection).
                append(") ;\n");
        iStrShaderBody.append("\t\t  float diff = max(dot(lightDir, norm), 0.0)").append(";\n");
        iStrShaderBody.append("\t\t  diffuseColor = ")
                .append(OpenGLProgramFactory.SHADER_VARIABLE_diffuseLightColor).append(" * diff").append(";\n ");
        //vec3 lightDir = normalize(-diffuseLight.direction);
        //float diff = max(dot(norm, lightDir), 0.0);
        //vec3 diffuse = light.diffuse * diff * texture(material.diffuse, TexCoords).rgb;
        if((shaderType & OpenGLProgramFactory.SHADER_VERTICES_WITH_DIFFUSE_TEXTURE) != 0){
            //texture is present so use it.
            iStrShaderDeclarations.append("uniform sampler2D ")
                    .append(OpenGLProgramFactory.SHADER_VARIABLE_diffuseTexture)
                    .append(";\n");
